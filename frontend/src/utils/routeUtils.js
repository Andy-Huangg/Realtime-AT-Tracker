...
export const createVehicleIcon = (
  ...
  switch (vehicleType?.toUpperCase()) {
    case "TRAIN":
      vehiclePath = trainPath;
      break;
    case "FERRY":
      vehiclePath = ferryPath;
      break;
    case "BUS":
    default:
      vehiclePath = busPath;
  ...
  const busTransform = `translate(${busX}, ${busY}) scale(${busSize / 24})`; // Scale based on 24x24 viewbox
  const svgString = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${iconWidth}" height="${iconHeight}" viewBox="0 0 ${iconWidth} ${iconHeight}">
      ...
      <!-- Vehicle Icon (Foreground) -->
      <g transform="${busTransform}">
         <path fill="white" d="${vehiclePath}" stroke="black" stroke-width="0.5" />
      </g>
    </svg>
  `;
